...
impl Statement {
    ...
    pub fn try_consume(tokens_queue: &mut TokenQueue, previous_queue_idx: &TokenQueueSlice, local_variables: &StackVariables, accessible_funcs: &FunctionList) -> Option<ASTMetadata<Statement>> {
        ...
        if let Some(ASTMetadata{resultant_tree, remaining_slice, extra_stack_used}) = IterationStatement::try_consume(tokens_queue, &curr_queue_idx, local_variables, accessible_funcs){
