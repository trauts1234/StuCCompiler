use crate::{args_handling::location_allocation::{generate_param_and_return_locations, AllocatedLocation, EightByteLocation, ReturnLocation}, asm_boilerplate::cast_from_acc, asm_gen_data::{AsmData, GetStructUnion}, assembly::{assembly::Assembly, operand::{immediate::{ImmediateValue, ToImmediate}, memory_operand::MemoryOperand, register::GPRegister, Operand, RegOrMem}, operation::AsmOperation}, compilation_state::label_generator::LabelGenerator, data_type::{base_type::{BaseType, FloatType, IntegerType, ScalarType}, recursive_data_type::{calculate_unary_type_arithmetic, DataType}}, debugging::ASTDisplay, expression::{expression::{self, Expression}, put_on_stack::PutOnStack}, expression_visitors::{data_type_visitor::GetDataTypeVisitor, expr_visitor::ExprVisitor, put_scalar_in_acc::ScalarInAccVisitor}, function_declaration::FunctionDeclaration, lexer::{punctuator::Punctuator, token::Token, token_savepoint::TokenQueueSlice, token_walk::{TokenQueue, TokenSearchType}}, parse_data::ParseData};
...
impl FunctionCall {
    ...
    /// sets up args and calls the function
    /// does not clobber or handle the return value
    fn call_function(&self, asm_data: &AsmData, stack: &mut SimpleStackFrame, global_asm_data: &crate::asm_gen_data::GlobalAsmData) -> (Assembly, Option<ReturnLocation>) {
        //system V ABI
        ...
        let type_matched_args: Vec<_> = self.args.iter()
            ...
            .map(|(i, expr)|{
                ...
                let param_type = if has_va_args && last_param_or_after{
                    ...
                    match expr.accept(&mut GetDataTypeVisitor {asm_data}).decay() {
        ...
        let (return_location, params_locations) = generate_param_and_return_locations(type_matched_args.iter().map(|(t, _)| t), &self.decl.return_type, asm_data, stack);
        ...
        for (dtype, expr, location) in memory_args_allocated {
            let calculate = put_arg_on_stack(expr, dtype.clone(), location, asm_data, stack, global_asm_data);
            result.merge(&calculate);
        }
...
impl PutOnStack for FunctionCall {
    fn put_on_stack(&self, asm_data: &AsmData, stack: &mut SimpleStackFrame, global_asm_data: &crate::asm_gen_data::GlobalAsmData) -> (Assembly, stack_management::stack_item::StackItemKey) {
        let (mut result, return_location) = self.call_function(asm_data, stack, global_asm_data);

        let resultant_location = match return_location {
            Some(ReturnLocation::InRegs(regs)) => {
                assert!([EightByteLocation::GP(GPRegister::_AX)] == regs[..]);//rax return only
                let resultant_location = stack.allocate(MemorySize::from_bytes(8));
                result.add_instruction(AsmOperation::MOV { to: RegOrMem::Mem(MemoryOperand::SubFromBP(resultant_location)), from: Operand::GPReg(GPRegister::_AX), size: MemorySize::from_bytes(8) });
...
fn put_arg_on_stack(expr: &Expression, arg_type: DataType,location: MemoryOperand, asm_data: &AsmData, stack_data: &mut SimpleStackFrame, global_asm_data:&crate::asm_gen_data::GlobalAsmData) -> Assembly {
    ...
    match (&param_type.decay(), &arg_type) {
        (DataType::RAW(BaseType::Struct(struct_name)), _) => {
            result.add_comment("putting struct arg on stack");
            //generate the struct
            let (struct_clone_asm, struct_stack_location) = expr.put_on_stack(asm_data, stack_data, global_asm_data);
            result.merge(&struct_clone_asm);
            //memcpy to the correct location
            result.add_instruction(AsmOperation::MEMCPY {
                size: asm_data.get_struct(struct_name).calculate_size().unwrap(),
