...
impl Typedef {
    pub fn try_consume(tokens_queue: &TokenQueue, previous_queue_idx: &TokenQueueSlice, scope_data: &mut ParseData, accessible_funcs: &FunctionList, struct_label_gen: &mut LabelGenerator) -> Option<ASTMetadata<(String, DataType, StorageDuration)>> {
        ...
        let ASTMetadata { remaining_slice: type_remaining, resultant_tree: (type_represented, storage_duration) } = consume_fully_qualified_type(tokens_queue, &type_slice, scope_data, accessible_funcs, struct_label_gen).unwrap();
