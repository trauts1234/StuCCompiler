use crate::{ast_metadata::ASTMetadata, data_type::recursive_data_type::DataType, function_declaration::consume_fully_qualified_type, lexer::{keywords::Keyword, punctuator::Punctuator, token::Token, token_savepoint::TokenQueueSlice, token_walk::{TokenQueue, TokenSearchType}}, parse_data::ParseData};
use unwrap_let::unwrap_let;
pub struct Typedef;

impl Typedef {
    pub fn try_consume(tokens_queue: &TokenQueue, previous_queue_idx: &TokenQueueSlice, scope_data: &mut ParseData) -> Option<ASTMetadata<(String, DataType)>> {
        ...
        let semicolon_idx = tokens_queue.find_closure_matches(&curr_queue_idx, false, |x| *x == Token::PUNCTUATOR(Punctuator::SEMICOLON), &TokenSearchType::skip_nothing()).unwrap().index;
        ...
        //get type
        let ASTMetadata { remaining_slice: type_remaining, resultant_tree: type_represented } = consume_fully_qualified_type(tokens_queue, &type_slice, scope_data).unwrap();
        assert!(type_remaining.get_slice_size() == 0);//must consume all of previous

        Some(ASTMetadata {
            remaining_slice: remaining,
            resultant_tree: (name, type_represented)
        })
    }
}