...
impl<'a> ExprVisitor for PopStructFromStack<'a> {
    ...
    fn visit_func_call(&mut self, func_call: &crate::function_call::FunctionCall) -> Self::Output {
        ...
        let return_value = func_call.accept(&mut GetDataTypeVisitor{asm_data: self.asm_data}).as_bare_struct().clone();
        let callee_name = &func_call.get_callee_decl().function_name;

        asm_line!(result, "add rsp, {} ; deallocate a struct returned from a function call to {}", return_value.calculate_size().unwrap().size_bytes(), callee_name);
